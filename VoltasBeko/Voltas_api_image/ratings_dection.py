...
def check_rgb_in_triangle(triangle_image):
    ...
    if count_red > 5500: #adjust based on use case(high>5000 if red fridge)
        return True
    else:
        return False
...
def mask_image(image):
    ...
    res=cv2.circle(mask, (int(h/2),int(w/2)), int(0.48*w), (255), -1)
    masked=cv2.bitwise_and(image,image,mask=res)
    res_image=masked[:int(w/2),:]
    return res_image
...
def ratings(image):
    ...
    roi_image = hsv_color_tracker(image)# , lower_hsv, upper_hsv
    roi_image=mask_image(roi_image)
    ...
    #plt.show()
    file_name = f"{star}_image_{current_time}.jpg"
